	...
	class DataSource {
		public:
			...
			vector<Game> getGameData() {
				...
				d.Parse(
					makeRequest("http://bridgesdata.herokuapp.com/api/games", 
			...
			vector<ActorMovieIMDB> getActorMovieIMDBData(int number = 0) {
				...
				d.Parse(makeRequest( url, {"Accept: application/json"}).c_str());
			...
			vector<EarthquakeUSGS> getEarthquakeUSGSData(int number = 0) {
				...
				if (number <= 0) {
					d.Parse(makeRequest( "http://earthquakes-uncc.herokuapp.com/eq",
										{"Accept: application/json"}).c_str());
					for (SizeType i = 0; i < d.Size(); i++) {
					...
					d.Parse(makeRequest( "http://earthquakes-uncc.herokuapp.com/eq/latest/" +
			 ...
			 * 		better)
			 */
			size_t curlWriteFunction(void *contents, size_t size, size_t nmemb, 
											void *results) {
