...
template <typename E> class Element
{
    ...
    protected:
        ...
        static string removeTrailingZeros(const double& num) {
            ...
            string numRep = to_string(num); numRep.erase(numRep.find_last_not_of("0")+1);//removes trailing 0s
            return numRep;
        }
        ...
        {//write out LinkVisualizer properties
            return OPEN_CURLY +
                QUOTE + "color"     + QUOTE + COLON + getCSSrep(lv.getColor())    + COMMA +
                QUOTE + "thickness" + QUOTE + COLON + removeTrailingZeros(lv.getThickness()) + COMMA +
                QUOTE + "weight"    + QUOTE + COLON + removeTrailingZeros(lv.getWeight()) + COMMA +
        ...
        {
            if(col.isTransparent()){return "[0, 0, 0, 0.0f]";}//leaves off other channels if transparent
            const string strCSS =
        ...
        {
            if (MAX_ELEMENTS_ALLOWED <= nodes.size()) // cant exceed max number of elements
            {
                throw "Max allowed elements(for visualization) exceeded.. "+to_string(nodes.size())+" Must be less than "+to_string(MAX_ELEMENTS_ALLOWED);
            }
            unordered_map<const Element*,int> map{{nullptr,-1}}; // map the nodes to a sequence of ids, 0...N-1 then get the JSON string for nodes placeholder nullptr prevents insertion of other nullptrs
            ...
            {
                if (map.emplace(e,i).second && ++i) // short circut only incriments i and gets rep upon successful emplacement
                {
                    nodes_JSON += e->getRepresentation() + COMMA;
                }
            }
            map.erase(nullptr); //Remove trailing comma and nullptr entry
            if (nodes_JSON.size()){nodes_JSON = nodes_JSON.erase(nodes_JSON.size()-1);}
            for (const auto& ele_int: map) //each pair<Element*,int> in map
            {
                for(const auto& ele_lv: ele_int.first->links) //each pair<Element*,LinkVisualizer> in Element*->links
                {
                    ...
                    {
                        links_JSON += getJSONrepresentation(ele_lv.second,to_string(ele_int.second),to_string(it->second)) + COMMA;
                    }
                }
            }
        ...
        static const pair<string,string>generateOrderedJSON(const vector<const Element*>& nodes)
        {
            if (MAX_ELEMENTS_ALLOWED <= nodes.size()) // cant exceed max number of elements
            {
                throw "Max allowed elements(for visualization) exceeded.. "+to_string(nodes.size())+" Must be less than "+to_string(MAX_ELEMENTS_ALLOWED);
