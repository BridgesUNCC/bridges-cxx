...
template<typename E> class ADTVisualizer {
	private:
...
//			adt_type.emplace("Array_Queue","llist");
			...
			adt_type.emplace("ArrayLlist", "AList");
		}
		...
		string getGraphAdjList_Representation(
			...
			list<SLelement<E>*> nodes, links_src, links_dest; 
			...
			for (auto& key_val: graph_adj_list){
				...
				while (a_list != NULL) {
					links_src.push_front(src_vertex);
					...
					links_dest.push_front(dest_vertex);
				...
				for (dle = first_element; dle!= NULL; dle = dle->getNext()) {
					...
					if (dle->getPrev() != NULL) {	// link exists
						...
						links.push_front(dle->getPrev());
									...
									// generate the JSON string
				return generateJSON_DLL(nodes, links);
		}
		
		/*
		string getALRepresentation() {
								// generate the JSON string
		...
		*/
		...
		string getTreeRepresentation(TreeElement<E> *root) {
			...
			list<TreeElement<E>*> nodes, links;
		...
		void preOrder(TreeElement<E> *root, 
				list<TreeElement<E>*> nodes, list<TreeElement<E>*> links){
...
cout << "Visualizer Type(build_JSON): " << visualizerType << endl;
			string s_final = 
