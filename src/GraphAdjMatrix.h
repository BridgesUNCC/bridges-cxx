 ...
 * Given the use of operator overloading, the adjacency matrix implementation is almost identical to a normal array representation,
...
class GraphAdjMatrix : public DataStructure
{
	...
	public:
         ...
         * If this key already exists its value and all associated edge values will be overwriten.
		...
		void addVertex(const K& k,const E& e=E())
		{
			...
			for(const auto& p : vertices){matrix[k][p.first] = matrix[p.first][k] = 0;}//set all its edge weights to 0
		}
        /**
		...
		void addEdge(const K& src,const K& dest,const unsigned int& wt)
		{
            try
            {
                ...
                else{vertices.at(src).links[&(vertices.at(dest))];} //creates default link data if none already present
