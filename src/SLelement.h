...
class SLelement : public Element<E> {
	...
	public:
	 	...
	 	*/
		SLelement(SLelement* n,const E& val = E(),const string& lab = string()) : 
							Element<E>(val,lab) {
			setNext(n);
		}
		...
		void setNext(SLelement* n) {
		    if (next!=n) {
				this->links.erase(next);//if different, remove old link data
			}
		    if ((next=n)){
					// set next to n and if not null, create default 
					// link data if none already present
				this->links[next];
			}
        }
        /**
	...
	private:
		 ...
		 */
		void orderHelper(unordered_set<const Element<E>*>& nodes) const {
			if(nodes.emplace(this).second && next){
				next->orderHelper(nodes);
			} 
