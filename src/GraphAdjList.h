...
class GraphAdjList : public DataStructure
{
    public:
        ...
        class Edge
        {
            ...
            public:
                /**
                 * Constructs an edge with the given destination vertex, weight, and edge data.
                ...
                /** Sets terminating vetex to "dest" @param dest The terminating vertex */
                void setVertex(const K& dest){vertex = dest;}
                /** @return The terminating vertex */
                K getVertex() const {return vertex;}
                /** Sets edge data to "data" @param data Application data */
	...
	public:
		...
		void addVertex(const K& k,const E& e= E())
		{
			...
			if(adj_list[k]){adj_list.at(k)->cleanup(); adj_list.at(k)=nullptr;}
		}
		/**
		...
		void addEdge(const K& src,const K& dest,const unsigned int& wt,const string& data = string())
		{
			try
			{
                ...
                vertices.at(src).links[&(vertices.at(dest))];//creates default link data if none already present
                stringstream conv; conv<<dest;
			    adj_list.at(src) = new SLelement<Edge>(adj_list.at(src),Edge(dest,wt,data),conv.str());
			}
    ...
    private:
		...
		virtual const pair<string,string> getDataStructureRepresentation(const unsigned int& arr_size) const override
		{
            ...
            {
                for(const auto& key_ele : this[i].vertices) //each pair<K,Element> in vertices
                {
