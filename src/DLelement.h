...
class DLelement: public SLelement<E>
{
	...
	public:
		/**
		 * Constructs a dlelement with the provided value, label, next and previous dlelements.
		 ...
		 * @param p The previous DLelement
	 	 */
		DLelement(DLelement* n,DLelement* p = nullptr,const E& val = E(),const string& lab = string()) : SLelement<E>(n,val,lab) {setPrev(p);}
		/**
		 * Constructs a dlelement with the provided value and label, setting the next and previous dlelements to NULL.
		 * The defaults will be used if not provided.
		 *
		 * @param val The data to hold
		 * @param lab The label to show
        ...
        /** Constant version */
        virtual const DLelement* getNext() const override {return static_cast<const DLelement*>(SLelement<E>::getNext());}
        /** Sets next to "n" @param n The next DLelement */
		void setNext(DLelement* n)
		{
		    if(this->next!=n && this->next!=prev){this->links.erase(this->next);} //if different, remove old link data
		    if((this->next=n)){this->links[this->next];} //set next to n and if not null, create default link data if none already present
        }
